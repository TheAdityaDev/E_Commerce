...
const Payment = () => {
    ...
    const finalize = async () => {
      ...
      try {
        ...
        const res = await axiosInstance.get(
          `/payment/${paymentId}?${query.toString()}`,
          {
            headers: {
              ...(token ? { Authorization: `Bearer ${token}` } : {}),
            },
  ...
  const openRazorpay = async () => {
    ...
    try {
      ...
      const options = {
        key:
          order.key || import.meta.env.REACT_APP_RAZORPAY_KEY || "RAZORPAY_KEY",
    ...
    >
      ...
      {status === "processing" && (
        <div>
          ...
          <p style={{ color: "#666" }}>
            If nothing opens, click below to open manually.
          </p>
          <button
            onClick={openRazorpay}
      ...
      {status === "success" && (
        <div>
          ...
          <p style={{ color: "#444" }}>
            Thank you! A receipt will be sent via SMS and email.
          </p>
      ...
      {status === "failure" && (
        <div>
          ...
          <p style={{ color: "#c00" }}>
            {message || "Your payment did not complete."}
          </p>
          <div style={{ marginTop: 12 }}>
            <button
              onClick={openRazorpay}
